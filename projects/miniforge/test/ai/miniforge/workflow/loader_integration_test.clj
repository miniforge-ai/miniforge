  ...
  (testing "complete workflow loading and validation flow"
    ...
    (let [result (loader/load-workflow :canonical-sdlc "2.0.0" {})]
      ...
      (let [workflow (:workflow result)]
        ...
        ;; New pipeline format uses :workflow/pipeline instead of :workflow/phases
        (is (vector? (:workflow/pipeline workflow)))
        (is (seq (:workflow/pipeline workflow)))
        ;; Verify pipeline entries have :phase keywords
        (is (every? :phase (:workflow/pipeline workflow))))
