...
(ns ai.miniforge.workflow.isolation-support
  "Keeps this project's pipeline-running tests off the developer's
   checkout and home.

   Project-level twin of the workflow brick's
   `ai.miniforge.workflow.isolation-test-support`, which explains the
   ...
   loadable from here — the same split `checkpoint-root-support` makes.
