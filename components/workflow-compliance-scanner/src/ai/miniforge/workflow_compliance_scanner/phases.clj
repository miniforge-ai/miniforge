...
(defn- resolve-scan-path
  "Resolve the path to scan — the repo/worktree the scanner reads files from.
   Prefers :execution/worktree-path (isolated copy), then input :repo-path, then '.'."
  [ctx]
...
(defn- resolve-output-path
  "Resolve the path for writing output reports — the actual repo, not the worktree.
   Prefers input :repo-path (explicit spec value), then '.' (CWD at invocation time).
   Never uses worktree-path so reports survive after the worktree is released."
  [ctx]
...
(defn enter-compliance-scan
  ...
  (let [start-time     (System/currentTimeMillis)
        repo-path      (resolve-scan-path ctx)
...
(defn enter-compliance-plan
  "Execute compliance plan phase.
   ...
   report to the output path (real repo, not worktree), stores plan in context."
  [ctx]
  (let [start-time     (System/currentTimeMillis)
        scan-path      (resolve-scan-path ctx)
        output-path    (resolve-output-path ctx)
        standards-path (resolve-standards-path ctx)
        classified     (get-in ctx [:execution/phase-results :compliance-classify :result :output :classified-violations] [])
        the-plan       (compliance-scanner/plan classified scan-path)]
    (compliance-scanner/write-work-spec! (:work-spec the-plan) output-path)
    (compliance-scanner/write-delta-report! output-path standards-path classified the-plan)
...
(defn enter-compliance-execute
  ...
  (let [start-time (System/currentTimeMillis)
        ...
        result     (compliance-scanner/execute! plan scan-path)]
