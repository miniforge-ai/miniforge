    ...
    (let [env-id      (random-uuid)
      ...
      (with-redefs [agent/create-implementer (fn [_] {:type :mock-implementer})
        ...
        (let [result (runner/run-pipeline
          ...
          (is (= env-id (get-in result [:execution/phase-results env-promotion-implement :result :environment-id]))
          ...
          (is (= env-id (get-in result [:execution/phase-results env-promotion-verify :result :environment-id]))
