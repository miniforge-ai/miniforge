  ...
  (let [mode (or execution-mode :local)]
    (try
      (let [create-registry (requiring-resolve 'ai.miniforge.dag-executor.executor/create-executor-registry)
        ...
        (if (nil? executor)
          (if (= :governed mode)
            (throw (ex-info "No capsule executor available for governed workflow"
                            {:execution-mode :governed
                             :hint "Start Docker or configure a Kubernetes cluster"}))
            nil)
   ...
   (let [;; Skip acquisition if executor already provided (e.g., sandbox setup)
         ...
         opts (if acquired-env
                (merge opts {:executor        (:executor acquired-env)
                             ...
                             :execution-mode  (or (:execution-mode acquired-env)
                                                  (:execution-mode opts))})
                opts)
