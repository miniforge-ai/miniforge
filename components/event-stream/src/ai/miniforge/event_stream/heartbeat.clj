...
(defn start-heartbeat!
  ...
  ([event-stream workflow-id phase-id opts]
   (let [interval-ms (get opts :interval-ms default-interval-ms)]
     ...
     ;; The scheduler's task calls core/phase-heartbeat + core/publish!,
     ;; both of which use swap!/swap-vals! on the event-stream atom.
     ;; Passing a non-IDeref stream (e.g. a WebSocket-backed map from
     ;; the dashboard layer) would only fail inside the scheduled task,
     ;; AFTER the executor is alive. Fail loud at the call site instead.
     (when-not (instance? clojure.lang.IDeref event-stream)
       (throw (ex-info "start-heartbeat!: event-stream must be IDeref (durable stream atom)"
                       {:event-stream-class (some-> event-stream class .getName)
                        :phase-id           phase-id
                        :workflow-id        workflow-id
                        :hint               "the scheduler updates the stream via swap!/swap-vals!; non-durable streams (e.g. WebSocket maps) need a different transport"})))
     (let [seq-num     (atom 0)
