...
;; ---------------------------------------------------------------------------
;; Layer 1.5 — agent-detail-panel
;; ---------------------------------------------------------------------------

(deftest agent-detail-panel-basic-rendering
  (let [agent (make-agent {:agent/name "Detail Agent" :agent/status :running})
        out   (str (sut/agent-detail-panel agent))]
    (testing "Shows agent name"
      (is (contains-substr? out "Detail Agent")))
    (testing "Shows vendor label"
      (is (contains-substr? out "Claude Code")))
    (testing "Shows status label"
      (is (contains-substr? out "Running")))
    (testing "Contains close button"
      (is (contains-substr? out "\u2715")))
    (testing "Contains Agent Info section"
      (is (contains-substr? out "Agent Info")))))

(deftest agent-detail-panel-truncates-id
  (let [id    (java.util.UUID/fromString "12345678-1234-1234-1234-123456789abc")
        agent (make-agent {:agent/id id})
        out   (str (sut/agent-detail-panel agent))]
    (is (contains-substr? out "12345678"))))

(deftest agent-detail-panel-pause-resume-terminate-actions
  (testing "Running agent has Pause and Terminate"
    (let [out (str (sut/agent-detail-panel (make-agent {:agent/status :running})))]
      (is (contains-substr? out "Pause"))
      (is (contains-substr? out "Terminate"))
      (is (not (contains-substr? out ">Resume<")))))
  (testing "Paused agent has Resume and Terminate"
    (let [out (str (sut/agent-detail-panel (make-agent {:agent/status :paused})))]
      (is (contains-substr? out "Resume"))
      (is (contains-substr? out "Terminate"))))
  (testing "Completed agent has no action buttons"
    (let [out (str (sut/agent-detail-panel (make-agent {:agent/status :completed})))]
      (is (not (contains-substr? out "Pause")))
      (is (not (contains-substr? out "Resume")))
      (is (not (contains-substr? out "Terminate"))))))

(deftest agent-detail-panel-capabilities-and-tags
  (let [agent (make-agent {:agent/capabilities #{:code-review :testing}
                           :agent/tags #{:backend :infra}})
        out   (str (sut/agent-detail-panel agent))]
    (testing "Shows capabilities"
      (is (contains-substr? out "Capabilities")))
    (testing "Shows tags"
      (is (contains-substr? out "Tags")))))

(deftest agent-detail-panel-no-capabilities-row-when-empty
  (let [agent (make-agent {:agent/capabilities #{} :agent/tags #{}})
        out   (str (sut/agent-detail-panel agent))]
    (is (not (contains-substr? out "Capabilities")))
    (is (not (contains-substr? out "Tags")))))

(deftest agent-detail-panel-session-metadata
  (let [agent (make-agent {:agent/metadata {:worktree "/tmp/wt" :branch "feat-1"}})
        out   (str (sut/agent-detail-panel agent))]
    (is (contains-substr? out "Session Metadata"))
    (is (contains-substr? out "worktree"))
    (is (contains-substr? out "/tmp/wt"))))

(deftest agent-detail-panel-no-metadata-section-when-empty
  (let [agent (make-agent {:agent/metadata {}})
        out   (str (sut/agent-detail-panel agent))]
    (is (not (contains-substr? out "Session Metadata")))))

(deftest agent-detail-panel-heartbeat-history
  (let [hb  [{:timestamp (java.util.Date.) :status :running :task "PR #1"}
              {:timestamp (java.util.Date.) :status :idle    :task "PR #2"}]
        out (str (sut/agent-detail-panel (make-agent {}) :heartbeat-history hb))]
    (is (contains-substr? out "Heartbeat History"))
    (is (contains-substr? out "PR #1"))
    (is (contains-substr? out "PR #2"))))

(deftest agent-detail-panel-no-heartbeat-history
  (let [out (str (sut/agent-detail-panel (make-agent {}) :heartbeat-history []))]
    (is (contains-substr? out "No heartbeat history available"))))

(deftest agent-detail-panel-pending-decisions
  (let [decisions [(make-decision {:decision/summary "Choose DB"})]
        out       (str (sut/agent-detail-panel (make-agent {})
                                               :pending-decisions decisions))]
    (is (contains-substr? out "Pending Decisions"))
    (is (contains-substr? out "Choose DB"))))

(deftest agent-detail-panel-fragment-delegates
  (testing "Fragment fn produces same content as panel fn"
    (let [agent (make-agent {})
          a (str (sut/agent-detail-panel agent))
          b (str (sut/agent-detail-panel-fragment agent))]
      (is (= a b)))))

;; ---------------------------------------------------------------------------
;; Layer 2 — status-filter-toolbar
;; ---------------------------------------------------------------------------

(deftest status-filter-toolbar-shows-all-button
  (let [agents [(make-agent {:agent/status :running})]
        out    (render-str (sut/status-filter-toolbar agents))]
    (is (contains-substr? out "All"))
    (is (contains-substr? out "(1)"))))

(deftest status-filter-toolbar-only-shows-nonzero-statuses
  (let [agents [(make-agent {:agent/status :running})
                (make-agent {:agent/status :running})
                (make-agent {:agent/status :blocked})]
        out    (render-str (sut/status-filter-toolbar agents))]
    (is (contains-substr? out "Running"))
    (is (contains-substr? out "(2)"))
    (is (contains-substr? out "Blocked"))
    (is (not (contains-substr? out "Idle")))))

(deftest status-filter-toolbar-invokes-js-function
  (let [out (render-str (sut/status-filter-toolbar [(make-agent {:agent/status :idle})]))] 
