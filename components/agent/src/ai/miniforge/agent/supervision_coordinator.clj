...
(ns ai.miniforge.agent.supervision-coordinator
  "Coordinator for workflow supervisors.

   The coordinator is a lightweight router, not a controller. It fans workflow
   state out to supervisors, aggregates their health decisions, and reports the
   resulting supervision status back to the workflow runtime."
  (:require [ai.miniforge.agent.supervision-protocol :as sp]
            [ai.miniforge.response.interface :as response]))

(defrecord SupervisionCoordinator
  [agents           ; Vector of Supervisor instances
   last-checks      ; Atom of {agent-id -> last-check-time}
   check-history])  ; Atom of [{:timestamp :agent-id :result}...]

(defn create-coordinator
  "Create a supervision coordinator.

   Arguments:
   - agents: Vector of Supervisor protocol instances

   Returns: SupervisionCoordinator record"
  [agents]
...
(defn should-check-agent?
  ...
  (if-not last-check-time
    ...
    (let [config (sp/get-supervisor-config agent)
...
(defn check-all-agents
  ...
  (let [agents (:agents coordinator)
        ...
        checks (for [agent agents
                 ...
                 (try
                   (let [result (sp/check-health agent workflow-state)]
        ...
        halt-check (first (filter sp/halt? checks))

        ;; Find any warnings
        warning-checks (filter sp/warning? checks)
...
(defn reset-all-agents!
  "Reset state of all supervisors. Called when workflow restarts."
  [coordinator]
  (doseq [agent (:agents coordinator)]
    (try
      (sp/reset-state! agent)
      (catch Exception _e
...
(defn add-agent!
  "Dynamically add a supervisor to the coordinator.
...
(defn remove-agent!
  ...
  (update coordinator :agents
          (fn [agents]
            (vec (remove #(= agent-id (-> % sp/get-supervisor-config :id))
                         agents)))))

(comment
  ;; Usage example
  ;; (require '[ai.miniforge.agent.supervision.progress-monitor :as pm])
  ;; (def progress-monitor (pm/create-progress-monitor-supervisor))
