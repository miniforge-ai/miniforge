...
(defn- ^{:stratum 0} make-waiver [eval-id]
  {:waiver/id    (random-uuid)
   :waiver/eval-id eval-id})

(defn- ^{:stratum 0} make-wf
...
(deftest ^{:stratum 0} derive-governance-state-no-evals-test
...
;; workflow-ticker
(deftest ^{:stratum 0} workflow-ticker-empty-model-test
  (testing "empty model returns empty vector"
    (is (= [] (sut/workflow-ticker {})))
    (is (= [] (sut/workflow-ticker {:workflows []})))))

;; pr-train-strip
(deftest ^{:stratum 0} pr-train-strip-empty-model-test
...
(deftest ^{:stratum 0} pr-train-strip-active-train-test
...
;; policy-health
(deftest ^{:stratum 0} policy-health-empty-prs-test
...
;; attention
(deftest ^{:stratum 0} attention-empty-model-test
  (testing "empty model returns empty vector"
    (is (= [] (sut/attention {})))
    (is (vector? (sut/attention {})))))

(deftest ^{:stratum 0} attention-failed-workflow-test
...
(deftest ^{:stratum 0} attention-explicit-items-test
...
(deftest ^{:stratum 1} derive-governance-state-filters-by-pr-id-test
...
(deftest ^{:stratum 1} workflow-ticker-returns-vector-test
  (testing "always returns a vector"
    (let [result (sut/workflow-ticker {:workflows [(make-wf (random-uuid) :running)]})]
      (is (vector? result)))))

(deftest ^{:stratum 1} workflow-ticker-row-shape-test
...
(deftest ^{:stratum 1} workflow-ticker-active-first-sort-test
...
(deftest ^{:stratum 1} pr-train-strip-counts-open-prs-test
...
(deftest ^{:stratum 1} pr-train-strip-counts-ready-prs-test
...
(deftest ^{:stratum 1} pr-train-strip-counts-monitored-prs-test
...
(deftest ^{:stratum 1} policy-health-pass-rate-calculation-test
...
(deftest ^{:stratum 1} policy-health-violation-categories-test
...
(deftest ^{:stratum 1} policy-health-governance-counts-test
...
(deftest ^{:stratum 1} attention-budget-exhausted-test
...
(deftest ^{:stratum 1} attention-budget-warning-test
...
(deftest ^{:stratum 1} attention-escalated-test
...
(deftest ^{:stratum 1} attention-severity-ordering-test
...
(deftest ^{:stratum 1} attention-always-returns-vector-test
