...
(ns ai.miniforge.tui-views.update.filter
  "Filter palette for the PR fleet view (VS Code cmd+p style).
   ...
   Layers 0-5 (over the 3-layer budget; Wave 2 namespace-split candidate)."
  (:require
...
(defn ^{:stratum 1} matches-text? [pr text]
  (or (str/blank? text) (fuzzy-match? (:pr/title pr) text)))

;------------------------------------------------------------------------------ Layer 2

(defn ^{:stratum 2} parse-filter-query
...
(defn ^{:stratum 3} matches-negated-fields? [pr parsed-filter]
  (every? (fn [[field-kw values]]
...
(defn ^{:stratum 4} pr-matches-filter?
...
;; Index computation
(defn ^{:stratum 5} compute-filter-indices
