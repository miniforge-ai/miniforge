...
(ns ai.miniforge.tui-views.persistence
  "Load persisted workflow events from disk on TUI startup.
   ...
   Layers 0-8 (over the 3-layer budget; Wave 2 namespace-split candidate).
   Pure data loading, no side effects beyond file I/O."
  (:require
...
(defn ^{:stratum 0} workflow-started-event [workflow-id spec]
  {:event/type :workflow/started
   :workflow/id workflow-id
   :workflow/spec spec})

(defn ^{:stratum 0} phase-started-event [workflow-id phase]
  {:event/type :workflow/phase-started
   :workflow/id workflow-id
   :workflow/phase phase})

(defn ^{:stratum 0} phase-completed-event [workflow-id phase outcome artifacts duration-ms
...
(defn ^{:stratum 0} agent-status-event [workflow-id agent-id status message]
...
(defn ^{:stratum 0} agent-chunk-event [workflow-id delta]
  {:event/type :agent/chunk
   :workflow/id workflow-id
   :chunk/delta delta})

(defn ^{:stratum 0} workflow-completed-event [workflow-id status duration-ms evidence-bundle-id
                                & [{:keys [tokens cost-usd]}]]
...
(defn ^{:stratum 0} workflow-failed-event [workflow-id error]
  {:event/type :workflow/failed
   :workflow/id workflow-id
   :workflow/failure-reason error
   :workflow/error-details {:message error}})

(defn ^{:stratum 0} gate-event [workflow-id gate passed? timestamp]
...
(defn ^{:stratum 0} update-phase-entry
...
(defn ^{:stratum 0} infer-artifact-type
...
(defn ^{:stratum 0} infer-artifact-name
...
(defn ^{:stratum 0} nested-dag-artifacts
...
(defn ^{:stratum 0} ensure-evidence-intent
  [detail event]
...
(defn ^{:stratum 0} append-validation-result
  [detail event passed?]
...
(defn- ^{:stratum 0} apply-agent-started
  [detail event]
...
(defn- ^{:stratum 0} apply-agent-completed
  [detail event]
  (let [agent (:agent/id event)
...
(defn- ^{:stratum 0} apply-agent-failed
  [detail event]
  (let [agent (:agent/id event)
...
(defn- ^{:stratum 0} apply-agent-status
  [detail event]
  (let [agent (:agent/id event)
...
(defn- ^{:stratum 0} apply-agent-chunk
  [detail event]
  (update detail :agent-output str (or (:chunk/delta event) "")))

(def ^{:stratum 0} ^:private stale-threshold-ms
...
(defn ^{:stratum 0} workflow->index-entry
...
(defn ^{:stratum 1} quick-named-workflow?
...
(defn ^{:stratum 1} workflow-terminal-event
  [events]
  (last (filter #(contains? lifecycle-event-types (:event/type %)) events)))

(defn ^{:stratum 1} top-level-workflow-events?
...
(defn ^{:stratum 1} normalize-artifact
  [artifact phase]
  (if (map? artifact)
...
(defn- ^{:stratum 1} apply-phase-started
  [detail event]
...
(defn- ^{:stratum 3} apply-phase-completed
  [detail event]
  (let [phase (event-phase event)
...
(defn- ^{:stratum 3} apply-workflow-completed
  [detail event]
  (-> detail
...
(defn- ^{:stratum 3} apply-workflow-failed
  [detail event]
  (-> detail
...
(defn ^{:stratum 3} derive-progress
...
(defn- ^{:stratum 3} archive-single-workflow
...
(defn ^{:stratum 4} apply-detail-event
  [detail event]
...
(defn ^{:stratum 4} archive-workflows!
...
(defn ^{:stratum 5} detail-from-events
  [workflow-id events]
  (reduce apply-detail-event (empty-detail workflow-id) events))

;------------------------------------------------------------------------------ Layer 6

(defn ^{:stratum 6} event-file->workflow
...
(defn ^{:stratum 6} load-workflow-detail
...
(defn ^{:stratum 7} load-workflows-indexed
...
(defn ^{:stratum 8} load-workflows-into-model
