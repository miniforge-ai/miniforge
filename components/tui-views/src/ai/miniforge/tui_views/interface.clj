  ...
  (when pr
    (let [checks (get pr :pr/ci-checks [])
          ci-str (if (seq checks)
                   (str "CI checks: "
                        (str/join ", "
                          (map #(str (:name %) "=" (name (or (:conclusion %) :unknown)))
                               checks)))
                   (str "CI status: " (name (or (:pr/ci-status pr) :unknown))))
          readiness (:pr/readiness pr)
          risk (:pr/risk pr)
          policy (:pr/policy pr)]
      (str "PR: " (:pr/repo pr) "#" (:pr/number pr) " — " (:pr/title pr) "\n"
           "Branch: " (:pr/branch pr) "\n"
           "Status: " (name (or (:pr/status pr) :unknown)) "\n"
           ci-str "\n"
           "Behind main: " (if (:pr/behind-main? pr) "yes" "no") "\n"
           (when readiness
             (str "Readiness score: " (:readiness/score readiness)
                  (when (:readiness/ready? readiness) " (ready)")
                  "\n"))
           (when risk
             (str "Risk level: " (name (or (:risk/level risk) :unknown))
                  (when-let [s (:risk/score risk)] (str " (score: " (format "%.2f" (double s)) ")"))
                  "\n"))
           (when policy
             (str "Policy: " (if (:evaluation/passed? policy) "passed" "FAILED")
                  (when-let [packs (:evaluation/packs-applied policy)]
