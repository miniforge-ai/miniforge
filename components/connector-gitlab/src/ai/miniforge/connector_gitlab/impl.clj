...
(defn- update-rate-limits!
  ...
  (when-let [info (http/parse-rate-headers response-headers gitlab-rate-headers)]
...
(defn- extract-project-notes
  ...
  (let [notes-resource (require-resource! "notes")
        notes         (->> (note-parent-records handle config auth-headers cursor opts)
                           ...
                           vec)]
    (connector/extract-result notes (http/max-timestamp-cursor timestamp-value notes) false)))
