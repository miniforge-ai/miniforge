...
Implements the `artifact` component for Phase 1 (Domain), providing artifact storage with provenance tracking using Datalevin in-memory database.

## Changes

- `components/artifact/deps.edn` - Datalevin 0.9.16 dependency
- `components/artifact/src/ai/miniforge/artifact/core.clj` - Core implementation with ArtifactStore protocol and Datalevin backend
