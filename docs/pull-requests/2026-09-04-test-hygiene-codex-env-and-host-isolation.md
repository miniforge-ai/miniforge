...
- default checkpoint root → a temp root, delegated to
  `checkpoint-test-support/call-with-temp-checkpoint-root` from #1890, which
  landed the checkpoint half of this fix first; the bundle archive is the
  part of `~/.miniforge/checkpoints` that #1890 does not cover
...
`meta-agent-test`, and the e2e namespace. Where #1890's `:each`
`with-temp-checkpoint-root` is already registered it stays; the two nest
without conflict, and a namespace with only this fixture is still fully
isolated.

The five project-level namespaces use a project-level twin,
`ai.miniforge.workflow.isolation-support` under `projects/miniforge/test`,
for the reason #1890 gives for `checkpoint-root-support`: `bb
test:integration` runs project tests on the project's own `deps.edn`
classpath, where a brick's test directory is not loadable. The two bodies
are identical below the `ns` form.
...
- Folding the two `isolation` twins and the two `checkpoint` twins into one
  loadable place (a test-support brick, or brick test dirs on the
  integration classpath) — four copies of two bodies is the cost of the
