...
Implements comprehensive pack dependency validation in the `knowledge-safety` policy pack as specified in N4 §2.4.2. Validates dependency graphs before pack loading to prevent circular dependencies, version conflicts, and trust escalation.

## Motivation

Spec enhancement PR #84 added `pack-dependency-validation` rule to the knowledge-safety policy pack. Without dependency validation:
