...
Systematic extraction of protocols from agent, artifact, llm, and loop components to establish clear separation between public extensibility points and internal implementation details, following the pattern demonstrated in the Redis codebase.
...
**Note:** Loop component implementation functions were not extracted to `impl/` namespaces in this PR since the existing functions are already well-structured and small. This may be done in a future refactoring if needed.
